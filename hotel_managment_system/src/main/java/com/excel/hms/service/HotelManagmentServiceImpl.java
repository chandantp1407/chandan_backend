...
public class HotelManagmentServiceImpl implements HotelManagmentService {
	...
	public List<GuestDto> getAllGuestDetails() {
		try {
			return guestRepository.findAll().stream().map(ObjectUtil::GuestEntityToDto).toList();
		} catch (Exception e) {
	...
	public List<RoomDto> getAllRooms() {
		try {
			return roomRepository.findAll().stream().map(ObjectUtil::RoomEntityToDto).toList();
		} catch (Exception e) {
	...
	public String saveReservation(ReservationDtoList dto) {
		...
		if (guestOptional.isPresent()) {
			...
			Optional<Reservation> activeReservation = guest.getReservations().stream()
					.filter(reservation -> !reservation.isCancelled() && !reservation.isClosed()).findFirst();
	...
	public List<ReservationDto> getAllReservations() {
		try {
			return reservationRepository.findAll().stream().map(ObjectUtil::ReservationEntityToDto).toList();
		} catch (Exception e) {
	...
	public List<AdminDTO> getAdmin() {
		return adminRepository.findAll().stream().map(x -> AdminDTO.builder().adminNo(x.getAdminNo())
				.adminName(x.getAdminName()).password(x.getPassword()).roleType(x.getRoleType()).build()).toList();
	}
	...
	public List<FeedbackDto> getAllData() {
		return feedbackRepository.findAll().stream().map(x -> FeedbackDto.builder().feedbackId(x.getFeedbackId())
				.name(x.getName()).email(x.getEmail()).subject(x.getSubject()).message(x.getMessage()).build())
				.toList();
	}
